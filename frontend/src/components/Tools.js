...
export default function Tools() {
  ...
  useEffect(() => {
    axios.get("https://portfolio-tfly.onrender.com/api/tools")
      ...
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-6 max-w-6xl mx-auto">
        {tools.map(tool => (
          <div key={tool._id} className="w-45 p-5 text-center hover:scale-105 transition-transform duration-300">
            {tool.icon && <img src={`https://portfolio-tfly.onrender.com${tool.icon}`} alt={tool.name} className="mx-auto mb-4 h-16" />}
